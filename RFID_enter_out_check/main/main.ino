...
#define EVENT_TIMEOUT_MS 3000 // YENİ: Son olayın ekranda kalma süresi (3 saniye)
...
int lastDay = -1;
unsigned long lastEventTimer = 0; // YENİ: Son olayın zamanını tutmak için zamanlayıcı
...
void setup() {
  ...
  Serial.println("\n-- RFID Access System - Stable Version with Event Timeout --");
...
void loop() {
  ...
  // YENİ: Son olayın üzerinden 3 saniye geçtiyse bilgileri sıfırla
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    lastEventUID = uid;
    lastEventName = name;
    lastEventTimer = millis(); // YENİ: Olay zamanını kaydet ve zamanlayıcıyı başlat
...
// --- Helper Functions ---
// ... (Bu bölümdeki diğer tüm fonksiyonlar önceki kodla aynıdır, değişiklik yoktur)
void checkForDailyReset() {
  struct tm timeinfo;
