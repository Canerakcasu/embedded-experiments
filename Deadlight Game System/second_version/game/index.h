...
<head>
    ...
    <style>
        ...
        #game-timer { font-size: 1.2em; color: var(--primary-color); margin-bottom: 10px; }
    </style>
</head>
<body>
    <div class="container">
        <div class="card">
            ...
            <div class="game-status-display" id="game-timer">Süre: 10:00</div>
            <div class="button-group">
                <button onclick="sendCommand('start_game')">OYUNU BAŞLAT</button>
                <button class="btn-danger" onclick="sendCommand('reset_game')">SİSTEMİ RESETLE</button>
            </div>
        </div>
        <div class="card">
            <h2>Canlı Takip</h2>
        ...
        <div class="card">
            ...
            <div class="control-group">
                ...
                <select id="language-select" onchange="sendSetting('set_sound_language', this.value)">
                    <option value="TR">Türkçe</option><option value="EN">İngilizce</option>
                </select>
            </div>
            <div class="control-group">
                <label>Ses Seviyesi: <span id="volume-value">5</span></label>
                <input type="range" id="volume-slider" min="0" max="30" value="5" oninput="sendSetting('set_volume', this.value)">
            </div>
        </div>
        <div class="card">
...
<script>
    ...
    function initWebSocket() {
        ...
        websocket.onclose = (event) => { console.log('Bağlantı kapandı, tekrar bağlanılıyor...'); setTimeout(initWebSocket, 2000); };
        websocket.onmessage = (event) => {
            ...
            // Update status dots using correct property names
            ...
            if(data.hasOwnProperty('soundLanguage')) { // Use 'soundLanguage' from JSON
                document.getElementById('language-select').value = data.soundLanguage;
            }
            if(data.hasOwnProperty('timer')) {
                const minutes = Math.floor(data.timer / 60);
                const seconds = data.timer % 60;
                document.getElementById('game-timer').innerText = `Süre: ${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
            }
        };
    }
    ...
    function sendSetting(setting, value) {
        ...
        if(setting === 'set_volume') document.getElementById('volume-value').innerText = value;
    }
</script>
</body>
</html>
